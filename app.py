...
# Tentukan password admin di sini (Silakan ganti sesuai kebutuhan Anda)
PASSWORD_ADMIN = "josjis"
...
def load_and_process_data_from_db():
    ...
    # --- HOT-FIX 1: KOREKSI DATA & WILAYAH ITS NU PEKALONGAN (4212) ---
    if 'id_sekolah' in df_sekolah.columns:
    ...
    # --- HOT-FIX 2: SUNTIKKAN DATA RUANGAN ITS NU PEKALONGAN (4212) ---
...
# ==================== KODE KEAMANAN ADMIN (PASWORD) ====================
with st.expander("⚙️ Konfigurasi Basis Data & Unggah File Master (Khusus Admin)", expanded=False):
    st.markdown("Bagian ini digunakan untuk melakukan update master data secara berkala.")
    
    # Input Password Admin
    input_password = st.text_input("🔑 Masukkan Password Admin untuk Mengakses Fitur:", type="password")
    
    if input_password == PASSWORD_ADMIN:
        st.success("🔓 Akses Diterima. Fitur Admin Terbuka.")
        st.markdown("---")
        ...
        if show_debug:
            try:
                ...
                st.markdown("**Status Kecocokan Ruang TAP & Ruang Sekolah:**")
        ...
        st.error("❌ Password salah! Fitur modifikasi database tetap terkunci.")
# =======================================================================
...
if data_siap_proses:
    try:
        ...
        # --- LOGIKA KUSTOM UNTUK FILTER SIDEBAR PENYESUAIAN TAP/S2 MENGGUNAKAN FUNGSI BARU ---
        # (Fungsi ini dipicu jika pengguna ingin meng-override kapasitas secara runtime di sidebar)
        def adjust_tap_capacity(df_ruangan):
            st.sidebar.subheader("⚙️ Penyesuaian Ruang TAP/S2 (Manual)")
            df = df_ruangan.copy()
            ...
            if mask_tap.any():
                for i, row in df[mask_tap].iterrows():
                    ...
                    df.at[i, 'kapasitas_5_sesi'] = sesi * kap
            return df
            
        # Panggil fungsi penyesuaian jika diperlukan di sidebar
        if st.sidebar.checkbox("Aktifkan Override Manual TAP/S2", value=False):
            df_ruang_raw = adjust_tap_capacity(df_ruang_raw)
        ...
        if df_filtered.empty:
            ...
            df_pivot_pct = df_filtered.pivot_table(index=['nama_sekolah'], columns='tgl_ujian', values='persentase_keterisian', aggfunc='mean')
            df_pivot_tap = df_filtered.pivot_table(index=['nama_sekolah'], columns='tgl_ujian', values='is_tap', aggfunc='max').fillna(False).astype(bool)
            ...
            if opsi_tampilan == "Tampilkan Sisa Kuota (Leb Jelas)":
                df_pivot = df_filtered.pivot_table(index=['nama_sekolah'], columns='tgl_ujian', values='sisa_kuota', aggfunc='sum')
                ...
                df_pivot = df_filtered.pivot_table(index=['nama_sekolah'], columns='tgl_ujian', values='persentase_keterisian', aggfunc='mean')
        ...
        with tab1:
            ...
            if not df_filtered.empty:
                ...
                fig = go.Figure()
                fig.add_trace(go.Bar(x=df_chart['nama_sekolah'], y=df_chart['kapasitas_total'], name='Daya Tampung Maksimal', marker_color='#6c757d', opacity=0.6))
                fig.add_trace(go.Bar(x=df_chart['nama_sekolah'], y=df_chart['total_peserta'], name='Total Peserta Terdaftar', marker_color='#007bff'))
                fig.update_layout(barmode='group', xaxis_title='Nama Lokasi', yaxis_title='Jumlah Kursi', legend_title='Keterangan', template='plotly_white', height=500)
        ...
        with tab3:
            ...
            st.download_button(label="📥 Unduh Laporan (.CSV)", data=csv_data, file_name=f"Laporan_Alokasi_UO_{pilih_kabupaten}.csv", mime="text/csv", use_container_width=True)
    ...
    1. **Masukkan Password Admin**: Ketik password pada kolom untuk membuka hak akses admin.
    2. **Upload Berkas Excel/CSV Baru**: Anda dapat mengunggah berkas data peserta baru serta data pelengkap lainnya.
    """)
